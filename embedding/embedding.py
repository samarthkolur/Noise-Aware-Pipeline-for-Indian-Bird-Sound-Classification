...
def _resolve_birdnet_model_path(cfg: dict) -> Path:
    ...
    if raw and str(raw).strip() and str(raw) != "auto":
        ...
        logger.info(
            "  [BirdNET] Model source: embedding.birdnet_model_path (explicit)"
        )
        return p
    ...
    if local_weights.is_file():
        logger.info(
            "  [BirdNET] Model source: project birdnet_weights/ (local copy)"
        )
    ...
    if bundled:
        ...
        logger.info(
            "  [BirdNET] Model source: birdnetlib site-packages (bundled .tflite)"
        )
...
class BirdNETEncoder(BaseEncoder):
    ...
    def __init__(self, cfg: dict) -> None:
        ...
        logger.info(
            "[BirdNET] Resolved model path: %s | load OK | encoder=BirdNET",
            self._model_path,
        )
        logger.info(
    ...
    def encode(self, waveform: np.ndarray, sr: int) -> np.ndarray:
        ...
        if not getattr(self, "_embedding_debug_printed", False):
            ...
            logger.info(
                "[BirdNET embedding] raw tensor shape: %s → flat shape: %s %s",
                raw_arr.shape,
                embedding.shape,
                embedding.dtype,
            )
            logger.info(
                "[BirdNET embedding] first 16 values: %s",
                embedding[:16],
            )
            logger.info(
                "[BirdNET embedding] mean=%.6f std=%.6f min=%.6f max=%.6f",
                mu,
                sigma,
                float(embedding.min()),
                float(embedding.max()),
            ...
            logger.info(
                "[BirdNET embedding] all_zeros=%s all_finite=%s dim_ok=%s",
                all_zero,
            ...
            if all_zero or sigma < 1e-12:
                logger.warning(
                    "[BirdNET embedding] vector is all zeros or ~constant — "
    ...
    def _report_cross_sample_embeddings(self, n: int) -> None:
        ...
        logger.info(
            "[BirdNET embedding] cross-sample check (n=%d): mean(std per dim across samples)=%.6f",
            n,
            mean_std_across_samples,
        )
        logger.info(
            "[BirdNET embedding] L2 between consecutive segments: min=%.6f max=%.6f mean=%.6f",
            float(l2_between_consecutive.min()),
            float(l2_between_consecutive.max()),
            float(l2_between_consecutive.mean()),
        )
        if identical or mean_std_across_samples < 1e-10:
            logger.warning(
                "[BirdNET embedding] embeddings are constant across samples — "
...
def streaming_embedding_scalar_mean_std(
    embeddings_dir: Path,
) -> Tuple[int, int, float, float]:
    """Global mean/std over all stored embedding elements without a full in-RAM stack.

    Numerically matches ``np.mean`` / ``np.std`` on the concatenated matrix
    (population standard deviation, ddof=0).

    Returns:
        (n_vectors, dim, global_mean, global_std)
    """
    total_elements = 0
    dim = 0
    sum_x = 0.0
    ...
    for species_dir in sorted(p for p in root.iterdir() if p.is_dir()):
        ...
        with h5py.File(h5_path, "r") as h5f:
            ...
            step = 512
            ch = ds.chunks
            if ch is not None and ch[0] is not None:
                step = max(step, int(ch[0]))
            for start in range(0, ds.shape[0], step):
                batch = np.asarray(ds[start : start + step], dtype=np.float64)
                n_vectors += batch.shape[0]
                total_elements += batch.size
                sum_x += float(batch.sum())
                sum_x2 += float(np.square(batch).sum())
    if total_elements == 0:
        return 0, dim, 0.0, 0.0
    mean = sum_x / total_elements
    var = sum_x2 / total_elements - mean * mean
    std = float(np.sqrt(max(var, 0.0)))
    return n_vectors, dim, mean, std
...
class EmbeddingPipeline:
    ...
    def _print_embedding_dataset_summary(self, manifest_path: Path, processed: int) -> None:
        ...
        logger.info(
            "[Embedding summary] encoder=%r | segments_written=%d | manifest_rows=%d",
            self.encoder.name,
            processed,
            n_manifest,
        )
        ...
        if n_manifest != processed:
            logger.warning(
                "[Embedding summary] manifest count (%d) != processed count (%d).",
                n_manifest,
                processed,
            )

        n_vec, dim, g_mean, g_std = streaming_embedding_scalar_mean_std(
            self.embeddings_dir
        )
        if n_vec == 0:
            logger.warning(
                "[Embedding summary] No vectors for mean/std (empty HDF5)."
            )
            return

        logger.info(
            "[Embedding summary] total_embedding_vectors=%d | dim=%d | "
            "global_mean=%.6f | global_std=%.6f (streaming over HDF5 chunks)",
            n_vec,
            dim,
            g_mean,
            g_std,
    ...
    def _log_manifest_species_counts(manifest_path: Path) -> None:
        ...
        logger.info(
            "[Embedding summary] manifest species: noise=%d, bird_and_species=%d, total_rows=%d",
            n_noise,
            n_other,
            len(rows),
        )
        if n_noise == 0:
            logger.warning(
                "[Embedding summary] no manifest rows with species='noise'. "
