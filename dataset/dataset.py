...
def make_weighted_sampler(
    ...
    *,
    ...
    Useful for oversampling minority classes during training. Pass *generator*
    (seeded) for reproducible sample order.
    """
...
def build_dataloaders(
    ...
    binary: bool = False,
    balanced_train: Optional[bool] = None,
    ...
    Args:
        ...
        balanced_train: If True, use ``WeightedRandomSampler`` on the train set.
            If None, read ``training.use_weighted_sampler`` (default False).
            **Do not combine** with BCE ``pos_weight`` or CE class weights: imbalance
            is then corrected twice. This repo defaults to loss-only reweighting
            (binary BCE ``pos_weight`` in ``Trainer``; multiclass CE weights in
            ``_build_loss``). Enable the sampler only if you intentionally disable
            those loss weights.
    ...
    if balanced_train is None:
        balanced_train = bool(train_cfg.get("use_weighted_sampler", False))
    ...
    # Train loader (optionally balanced — see balanced_train docstring)
    train_subset = Subset(dataset, splits.train_idx)
    if balanced_train:
        seed = int(cfg.get("project", {}).get("seed", 42))
        sampler_gen = torch.Generator().manual_seed(seed)
        sampler = make_weighted_sampler(
            dataset.labels, splits.train_idx, generator=sampler_gen
        )
