...
class Miner:
    ...
    def mine_false_positives(self, source_dir: Path, export_dir: Path) -> None:
        ...
        for audio_path in tqdm(sorted(audio_files), desc="Mining"):
            try:
                # Use Predictor to run the pipeline without writing to outputs/ immediately
                # Predictor.predict_file now yields results and does inline routing, 
                # but we will just intercept the outputs/clean_birds directory!
                # Since Predictor hardcodes self.birds_dir, we will temporarily override it.
                
                original_birds_dir = self.predictor.birds_dir
                original_noise_dir = self.predictor.noise_dir
                
                self.predictor.birds_dir = export_dir
                
                # We don't care about the true negatives for mining review
                import tempfile
                with tempfile.TemporaryDirectory() as dump_dir:
                    self.predictor.noise_dir = Path(dump_dir)
                    results = self.predictor.predict_file(audio_path)
                ...
                self.predictor.birds_dir = original_birds_dir
                self.predictor.noise_dir = original_noise_dir
